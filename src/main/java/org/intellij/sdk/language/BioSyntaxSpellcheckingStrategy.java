...
final class BioSyntaxSpellcheckingStrategy extends SpellcheckingStrategy {

  @Override
  public @NotNull Tokenizer<?> getTokenizer(PsiElement element) {
    if (element instanceof PsiComment) {
      return new BioSyntaxCommentTokenizer();
    }

    if (element instanceof BioSyntaxProperty) {
      return new BioSyntaxPropertyTokenizer();
    }

    return EMPTY_TOKENIZER;
  }

  private static class BioSyntaxCommentTokenizer extends Tokenizer<PsiComment> {
  ...
  private static class BioSyntaxPropertyTokenizer extends Tokenizer<BioSyntaxProperty> {

    public void tokenize(@NotNull BioSyntaxProperty element, @NotNull TokenConsumer consumer) {
      //Spell check the keys and values of properties with different splitters
      final ASTNode key = element.getNode().findChildByType(BioSyntaxTypes.KEY);
      ...
      final ASTNode value = element.getNode().findChildByType(BioSyntaxTypes.VALUE);
